...
export default function EditAnnouncement({ announcement }) {
  ...
  const handleEdit = () => {
    ...
    idInput.current.value = announcement.id
    editDateInput.current.value = new Date()
  ...
  if (status === 'authenticated') {
    ...
    return (
      <main className="max-h-[100vh] flex-auto flex flex-col items-center justify-start p-4 bg-white">
        ...
        <form ref={editFormRef} action="/api/pengumuman/edit" method="post" onSubmit={(e) => e.preventDefault()} className="flex-auto w-full flex overflow-y-auto">
          <TiptapTemplate>
            <div className="flex flex-col w-full bg-slate-100">
              ...
              <div className={`${MenuBarStyles} items-center content-center`}>
                <MenuButtons editor={editor}>
                  <button onClick={handleEdit} className="px-4 py-2 text-lg font-bold bg-blue-500 hover:bg-blue-700 text-white rounded-md">
                    Simpan
                  </button>
                  <button onClick={() => setHidden(false)} className="px-4 py-2 text-lg font-bold bg-red-500 hover:bg-red-700 text-white rounded-md">
                    Hapus
                  </button>
                </MenuButtons>
              </div>
            ...
            <input ref={idInput} type="hidden" name="id" value="" />
